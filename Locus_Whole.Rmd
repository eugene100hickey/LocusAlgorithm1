...
To illustrate the workings of the Locus Algorithm, a worked example is given here. The star with SDSS ID `r ObjID` (RA = `r round(ra, 3)` and DEC = `r round(dec, 3)`) is used as the example. This star, in the constellation Aquarius, has SDSS magnitudes as given in the table below.
...
\begin{flushleft}The locus of the target is, by equation 2 above, \end{flushleft}

\center `r signif(ra - ra.size/2, 5)` $\leq$ RA $\leq$ `r signif(ra + ra.size/2, 5)` \center
\center `r signif(dec - dec.size/2, 5)` $\leq$ Dec $\leq$ `r signif(dec + dec.size/2, 5)` \center

\begin{flushleft}The candidate zone is, by equation 3 above, \end{flushleft}

\center `r signif(ra - ra.size, 5)` $\leq$ RA $\leq$ `r signif(ra + ra.size, 5)` \center
\center `r signif(dec - dec.size, 5)` $\leq$ Dec $\leq$ `r signif(dec + dec.size, 5)` \center

\newpage

## Identification and Filtering of Reference Stars

\begin{flushleft}The potential reference stars are selected as follows:\end{flushleft}
...
int.pts %>% ggplot(aes(x = -int_ra, y = int_dec)) + 
  geom_point(fill = "black", colour = "black", size = 0.5) + #intersection points in black
...
After checking different fields of view, a pointing with RA = `r round(final_pointing$ra, 4)` and DEC = `r round(final_pointing$dec, 4)` included both the target and `r dim(B)[1]` reference stars. This pointing is calculated to have a score of `r signif(final_pointing$score, 3)`
\newpage
