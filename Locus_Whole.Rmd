...
The star with SDSS ID `r ObjID`, henceforth called the target, (RA = `r round(ra, 4)` and DEC = `r round(dec, 4)`) is used as the example. This star, in the constellation Aquarius, has SDSS magnitudes as given in the table below.
...
telescope <- data.frame(Parameters = c("Field of View in degrees",
                                       "Resolution Limit in degrees",
                                       "Dynamic Range in magnitudes",
                                       "Colour Match Limit"),
                        Values = c(dec.size*1, resol*1, dynamic_range, M)) 
telescope %>%  kable()
```
...
\center = `r round(dec.size, 4)` / cos(`r round(dec, 4)` $^{\circ}$) \center

\center = `r signif(dec.size / cos(dec*pi/180), 5)` $^{\circ}$ \center


The locus of the target is, by equation 2 above,

\center `r round(ra - ra.size/2, 4)` $\leq$ RA $\leq$ `r round(ra + ra.size/2, 4)` \center
\center `r round(dec - dec.size/2, 4)` $\leq$ Dec $\leq$ `r round(dec + dec.size/2, 4)` \center
...
\center `r round(ra - ra.size, 4)` $\leq$ RA $\leq$ `r round(ra + ra.size, 4)` \center
\center `r round(dec - dec.size, 4)` $\leq$ Dec $\leq$ `r round(dec + dec.size, 4)` \center
...
* Position: Within the Candidate Zone, SDSS records `r Object_Count` separate objects with clean photometry, (@Aguado2018).
...
The table below gives the `r object_count_crowding` stars in the candidate zone along with the highlighted `r dim(B)[1]` stars in the final field of view.

\vskip 0.2in

```{r reference_table_all, echo=F, message=F, warning=F}
options(knitr.table.format = "latex")
final_stars <- A$objID %in% B$objID
A %>%
...
Where $\delta r$ stand for the difference between the r-band magnitude for the target and the reference star and M is the colour match limit.
...
\vskip 0.2in
`r  round(A$ra[corner1], 4) ` `r ifelse(A$ra[corner1]>ra, print(">"), print("<"))` `r round(ra, 4)` 
=> RA~c~ =  `r  round(A$ra[corner1], 4)` `r ifelse(A$ra[corner1]>ra, print("-"), print("+"))` `r round(ra.size/2, 4) `  = `r  round(ifelse(A$ra[corner1]>ra, A$ra[corner1] - ra.size/2, A$ra[corner1] + ra.size/2), 4) `
\vskip 0.2in
`r  round(A$dec[corner1], 4) ` `r ifelse(A$dec[corner1]>dec, print(">"), print("<"))` `r round(dec, 4)` 
=> Dec~c~ =  `r  round(A$dec[corner1], 4)` `r ifelse(A$dec[corner1]>dec, print("-"), print("+"))` `r round(dec.size/2, 4) `  = `r  round(ifelse(A$dec[corner1]>dec, A$dec[corner1] - dec.size/2, A$dec[corner1] + dec.size/2), 4) `


From Expression 9:
RA~r~ `r ifelse(A$ra[corner1]>ra, print(">"), print("<"))` RA~t~ =>  DirRA = `r ifelse(A$ra[corner1]>ra, print("-"), print("+"))`ive
Dec~r~ `r ifelse(A$dec[corner1]>dec, print(">"), print("<"))` Dec~t~ =>  DirDec = `r ifelse(A$dec[corner1]>dec, print("-"), print("+"))`ive
\vskip 0.2in
For reference SDSS`r A$objID[corner1]` (Star `r rownames(A)[corner1]` on figure N)
\vskip 0.2in
`r  round(A$ra[corner1], 4) ` < `r round(ra, 4)` => DirDec = +ive,   
`r  round(A$dec[corner1], 4) ` < `r round(dec, 4)`   =>  DirRA = +ive
\vskip 0.2in
Repeating this calculation for  SDSS`r A$objID[corner2]` (Star `r rownames(A)[corner2]` on figure N) gives 
RA~c~ = `r  ifelse(A$ra[corner2]>ra, round(A$ra[corner2] - ra.size/2, 4), round(A$ra[corner2] + ra.size/2, 4)) `, 
Dec~c~ = `r  ifelse(A$dec[corner2]>dec, round(A$dec[corner2] - dec.size/2, 4), round(A$dec[corner2] + dec.size/2, 4)) `, 
\vskip 0.2in
DirRA = `r ifelse(A$ra[corner2]>ra, print("-"), print("+"))`ive, DirDec = `r ifelse(A$dec[corner2]>dec, print("-"), print("+"))`ive.  

We can compare these two effective loci to see if they intersect to form a valid PoI.
\vskip 0.2in
From Expression 10:
DirRA`r rownames(A)[corner1]` = +ive, DirDec`r rownames(A)[corner2]`=+ive
RA~r~`r rownames(A)[corner1]`> RA~r~`r rownames(A)[corner1]` AND Dec~r~`r rownames(A)[corner1]` < Dec~r~`r rownames(A)[corner1]`

PoI exist at (RA~c~`r rownames(A)[corner1]`, Dec~c~`r rownames(A)[corner1]`) =  (`r round(int.pts$int_ra[1], 4)`,  `r round(int.pts$int_dec[1], 4)`)
\vskip 0.2in
From Expression 2, the FoV centred on the PoI (`r round(int.pts$int_ra[1], 4)`,  `r round(int.pts$int_dec[1], 4)`) can be calculated to be:

$$RA_c - {\frac{R^`}{2}} \leq RA \leq RA_c + {\frac{R^`}{2}} $$

$$Dec_c - {\frac{S}{2}} \leq Dec \leq Dec_c + {\frac{S}{2}}$$

`r round(int.pts$int_ra[1], 4)` - `r ra.size` $\leq$ RA $\leq$ `r round(int.pts$int_ra[1], 4)` + `r ra.size` 
\vskip 0.2in
`r round(int.pts$int_dec[1], 4)` - `r dec.size` $\leq$ Dec $\leq$ `r round(int.pts$int_dec[1], 4)` + `r dec.size` 
\vskip 0.2in
`r round(int.pts$int_ra[1] - ra.size, 4)` $\leq$ RA $\leq$ `r round(int.pts$int_ra[1] + ra.size, 4) ` 
\vskip 0.2in
`r round(int.pts$int_dec[1] - dec.size, 4)` $\leq$ Dec $\leq$ `r round(int.pts$int_dec[1] + dec.size, 4)`
\newpage
...
int.pts %>% ggplot(aes(x = -int_ra, y = int_dec)) + 
  ...
  geom_segment(data = loci_horizontal, aes(x = -x1, y = y1, xend = -x2, yend = y2),
            colour="blue", size = 0.25) + ### locii in blue
...
```{r reference_table, warning = F, message = F, echo = F, include = T}
B %>% 
  rownames_to_column(var = "ref.") %>% 
  mutate_if(is.numeric, round, 3) %>% 
  rename_at(vars(starts_with("psfmag")), funs(str_replace(., "psfmag", "mag"))) %>% 
