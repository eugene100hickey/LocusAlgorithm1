...
`r round(int.pts$int_ra[1], 4)` - `r round(ra.size/2, 4)` $\leq$ RA $\leq$ `r round(int.pts$int_ra[1], 4)` + `r round(ra.size/2, 4)` 
\vskip 0.2in
`r round(int.pts$int_dec[1], 4)` - `r round(dec.size/2, 4)` $\leq$ Dec $\leq$ `r round(int.pts$int_dec[1], 4)` + `r round(dec.size/2, 4)` 
\vskip 0.2in
`r round(int.pts$int_ra[1] - ra.size/2, 4)` $\leq$ RA $\leq$ `r round(int.pts$int_ra[1] + ra.size/2, 4) ` 
\vskip 0.2in
`r round(int.pts$int_dec[1] - dec.size/2, 4)` $\leq$ Dec $\leq$ `r round(int.pts$int_dec[1] + dec.size/2, 4)`
\newpage
